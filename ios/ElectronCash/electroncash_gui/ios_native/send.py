...
class SendVC(SendBase):
    ...
    def doUpdateFee(self) -> None:
        ...
        if amount is None:
            ...
            if not outputs:
                ...
                outputs = [(_type, addr, amount)]
            try:
                tx = wallet().make_unsigned_transaction(get_coins(self), outputs, config(), fee, sign_schnorr=False)
    ...
    def showTransaction_desc_(self, txraw, desc) -> None:
        tx = Transaction(txraw, sign_schnorr=False)
        tx.deserialize()
    ...
    def doSend_(self, preview : bool) -> None:
        ...
        try:
            tx = wallet().make_unsigned_transaction(coins, outputs, config(), fee, sign_schnorr=False)
        except NotEnoughFunds:
